...
//  ----- LANG SWITCHER -----
	$('.lang-switcher__list').click(function(){
		$(this).toggleClass('open');
	});

	let html             = document.getElementsByTagName("html")[0],
		htmlLang         = html.getAttribute("lang"),
		switcherItem     = document.getElementsByClassName("lang-switcher__item"),
		switcherItemLang = [];

	for (let i = 0; i<switcherItem.length; i++) {
		switcherItemLang[i] = switcherItem[i].getAttribute("data-lang");
	}

	for (let i = 0; i<switcherItemLang.length; i++) {
		if (switcherItemLang[i]==htmlLang) {
			switcherItem[i].classList.add("lang-switcher__item--active");
		}
	}
